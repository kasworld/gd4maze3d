...
func init(st :Storey) -> MiniMap:
	...
	storey = st
	walls_known = []
	walls_known.resize(storey.maze3d_setting.MazeSize.y*2+1)
	for cl in walls_known:
		cl.resize(storey.maze3d_setting.MazeSize.x*2+1)
...
func get_width() -> float:
	return storey.maze3d_setting.MazeSize.x * map_scale
func get_height() -> float:
	return storey.maze3d_setting.MazeSize.y * map_scale
